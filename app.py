...
from scoring_engine import score_portfolio, get_action, get_action_color
...
with st.sidebar:
    ...
    # Min. açık tutar
    min_amount = st.number_input("Min. Açık Tutar (USD)", min_value=0, value=0, step=1000)
...
with col1:
    st.metric("Toplam Borçlu", f"{total_debtors}", help="Portföydeki toplam borçlu sayısı")
...
# AKSİYON DAĞILIMI
# ──────────────────────────────────────────────────────────────
st.markdown('<div class="section-title">⚡ Bugünün Aksiyonları</div>', unsafe_allow_html=True)
...
for col, (action, color, hex_color) in zip([col_a1, col_a2, col_a3, col_a4], action_data):
...
# BORÇLU TABLOSU
# ──────────────────────────────────────────────────────────────
st.markdown(f'<div class="section-title">📋 Borçlu Tablosu ({len(filtered_df)} sonuç)</div>', unsafe_allow_html=True)
...
display_df['Ödeme Geçmişi'] = display_df['Ödeme Geçmişi'].apply(lambda x: f"{x:.0f}/100")
...
export_df = filtered_df[[
    'debtor_id', 'debtor_name', 'risk_score', 'action', 'action_en',
    'days_overdue', 'outstanding_amount', 'payment_history_score',
    'days_since_contact', 'trend_label', 'last_contact_date'
]].copy()
...
# ──────────────────────────────────────────────────────────────
st.markdown('<div class="section-title">🔬 Detaylı Borçlu Analizi</div>', unsafe_allow_html=True)
...
if debtor_options:
    ...
    with col_d2:
        ...
        trend_color = "#fc8181" if debtor_row['score_trend'] > 0 else ("#68d391" if debtor_row['score_trend'] < 0 else "#90cdf4")
    ...
    # LLM Açıklama
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("**🤖 AI Karar Açıklaması** *(Groq · Llama 3.3 70B · Karar vermez, açıklar)*")
    ...
    if generate_btn:
        with st.spinner(f"Llama 3.3 70B ile {debtor_row['debtor_name']} için açıklama üretiliyor..."):
